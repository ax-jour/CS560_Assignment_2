...
def RotatedTransDist(point1, point2):
    rotate_diff = point1[2] - point2[2]

    # Wrapping the angle with in -pi to pi
    if rotate_diff > math.pi:
        rotate_diff -= 2 * math.pi
    elif rotate_diff < -math.pi:
        rotate_diff += 2 * math.pi

    return math.sqrt((point1[0] - point2[0]) ** 2 + (point1[1] - point2[1]) ** 2 + rotate_diff ** 2)
...
def str2doubleInNestedLst(nestedLst):
    rtnLst = []
...
def polyTriangulate(obstacle):
    ...
    point_count = len(temp)
    for i in range(point_count):
        prev_index = i - 1
        prev_point = temp[prev_index]
        point = temp[i]
        next_index = (i + 1) % point_count
        next_point = temp[next_index]

        if isEar(prev_point, point, next_point, temp):
    ...
    while ear_vertex and point_count >= 3:
        ...
        if point_count > 3:
            prev_prev_point = temp[prev_index - 1]
            next_next_index = (i + 1) % point_count
            next_next_point = temp[next_next_index]

            groups = [
                (prev_prev_point, prev_point, next_point, temp),
                (prev_point, next_point, next_next_point, temp),
            ]
            for group in groups:
                p = group[1]
                if isEar(*group):
...
def isEar(p1, p2, p3, obstacle):
    ear = containsNoPts(p1, p2, p3, obstacle) and \
          isConvex(p1, p2, p3) and \
          triangleArea(p1[0], p1[1], p2[0], p2[1], p3[0], p3[1]) > 0
    return ear
