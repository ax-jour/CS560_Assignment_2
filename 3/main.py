...
if __name__ == '__main__':
    ...
    # path = rrt(rob, obstacles, start, goal, 0, 10, 0.5, 5, 1000)
    # if path is not None:
    #     visualize_trajectory(rob, obstacles, start, goal, path, 0.5)
    # plt.show()


    ######################################################################
    ### TESTs RRT algorithms of Kinematics car
    ######################################################################
    ...
    for n in range(rounds):
        ...
        for i in range(total):
            ...
            while True:
                temp_start = sample_point()
                temp_end = sample_point()
                if isCollisionFree(rob, temp_start, obstacles) and isCollisionFree(rob, temp_end, obstacles):
                    break

            rrt_start = time.time()
            path = rrt(rob, obstacles, start, goal, n1, n2, dt, sample_time, iter_n)
            rrt_end = time.time()
            if len(path) > 0:
                rrt_passed += 1
